...
async function streamWithRetries(
  ...
  for (let attempt = 0; attempt <= RETRY_DELAYS_MS.length; attempt += 1) {
    ...
    try {
      return await createStreamingResponse(client, model, input, timeoutMs);
    } catch (error) {
      lastError = error;

      if (error instanceof RateLimitError && attempt === 0) {
        try {
          return await createStreamingResponse(client, fallbackModel, input, timeoutMs);
        } catch (fallbackError) {
...
export async function POST(request: Request) {
  ...
  const fallbackModel = process.env.OPENAI_FALLBACK_MODEL ?? "gpt-4.1-nano";
  ...
  const input = buildOpenAiInput(history, telemetryMessage);

  const client = new OpenAI({ apiKey });

  try {
    const stream = await streamWithRetries(client, primaryModel, fallbackModel, input, timeoutMs);
